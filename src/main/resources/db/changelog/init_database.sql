...
(
    ...
    call_fee_id       bigint      not null,
    call_fee_range_id bigint      not null,
    call_date         datetime    not null,
    duration          int         not null,
    total             float       not null,
    bill_id           bigint,
    constraint pk_calls primary key (call_id),
    constraint fk_calls_clients_org foreign key (phone_origin) references clients (phone_number) on delete restrict on update cascade,
    constraint fk_calls_clients_dest foreign key (phone_destination) references clients (phone_number) on delete restrict on update cascade,
    constraint fk_calls_fees foreign key (call_fee_id) references calls_fees (call_fee_id) on delete restrict on update cascade,
    constraint fk_calls_ranges foreign key (call_fee_range_id) references calls_fees_ranges(call_fee_range_id) on delete restrict on update cascade,
